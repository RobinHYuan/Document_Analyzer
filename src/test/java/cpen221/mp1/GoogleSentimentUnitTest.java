...
public class GoogleSentimentUnitTest {
    ...
    public void testSentimentsPos() {
        try {
            Assertions.assertEquals("\"Well, try dancing now!\"", SentimentAnalysis.getMostPositiveSentence(testDocument1));
    ...
    public void testSentimentsNeg() {
        try {
            Assertions.assertEquals("Then the snow fell and she could find nothing at all to eat.", SentimentAnalysis.getMostNegativeSentence(testDocument1));
        } catch (NoSuitableSentenceException e) {
            throw new RuntimeException(e);
        }
    }
